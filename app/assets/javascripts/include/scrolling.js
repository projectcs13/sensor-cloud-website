  ...
  if(type=="stream"){
	   $.ajax({
  		  ...
  		  data: { search: { page: stream_count, sort_by: $('#search_sort_by').val(), query: $('#search_query').val(), 
                                              filter_tag: $('#search_filter_tag').val(), filter_unit: $('#search_filter_unit').val(), filter_longitude: $('#long').val(), 
                                              filter_latitude: $('#lati').val(), filter_distance: $('#rad').val(),refresh: false}}
	   }).done(function( msg ) {
    	  waiting = false;
